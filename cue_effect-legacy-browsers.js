...
let expName = 'cue_effect';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  margin = new visual.ImageStim({
    ...
    image : 'resources/circle.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, pos : [0, 0], size : [0.5, 0.5],
    color : new util.Color([1,1,1]), opacity : undefined,
    flipHoriz : false, flipVert : false,
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.5 && random_image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 0.5 + 0.9 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (random_image.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      random_image.setAutoDraw(false);
    }
    
    // *margin* updates
    if (t >= 1.4 && margin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 1.4 + 0.9 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (margin.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      margin.setAutoDraw(false);
    }
    
    // *image* updates
    if (t >= 2.3 && image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 2.3 + appearance_time - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (image.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      image.setAutoDraw(false);
    }
    
    // *key_resp* updates
    if (t >= 2.3 && key_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
