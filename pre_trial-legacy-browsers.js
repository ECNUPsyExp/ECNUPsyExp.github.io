...
let expName = 'pre_trial';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'conditions_pretrial.xlsx', 'path': 'conditions_pretrial.xlsx'},
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials = new TrialHandler({
      ...
      trialList: 'conditions_pretrial.xlsx',
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    frameRemains = 1.5 + 0.6 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (random_image.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      random_image.setAutoDraw(false);
    }
    
    // *margin* updates
    if (t >= 2.1 && margin.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *image* updates
    if (t >= 2.7 && image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 2.7 + appearance_time - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (image.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      image.setAutoDraw(false);
    }
    
    // *key_resp* updates
    if (t >= 2.7 && key_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
